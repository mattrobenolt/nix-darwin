...

{
  ...
  launchd = {
    ...
    user.agents = {
      ...
      # llama-swap: OpenAI-compatible proxy for automatic model loading/unloading
      # Config: ~/.config/llama-swap/config.yaml (generated by Home Manager)
      # Only runs if config file exists.
      llama-swap = {
        serviceConfig = {
          Label = "com.mattrobenolt.llama-swap";
          ProgramArguments = [
            "${pkgs.llama-swap}/bin/llama-swap"
            "-config"
            "/Users/matt/.config/llama-swap/config.yaml"
          ];
          EnvironmentVariables = {
            HOME = "/Users/matt";
            PATH = "${pkgs.llama-cpp}/bin:/usr/bin:/bin";
          };
          KeepAlive = {
            PathState = {
              "/Users/matt/.config/llama-swap/config.yaml" = true;
            };
          };
          RunAtLoad = true;
          StandardOutPath = "/Users/matt/Library/Logs/llama-swap.log";
          StandardErrorPath = "/Users/matt/Library/Logs/llama-swap.log";
        };
      };
    }; # user.agents
  };
  ...
  system.activationScripts.postActivation.text = ''
    ...
    /bin/launchctl kickstart -k "gui/$(id -u matt)/com.mattrobenolt.llama-swap" 2>/dev/null || true
